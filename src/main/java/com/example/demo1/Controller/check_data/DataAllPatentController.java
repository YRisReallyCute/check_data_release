...
@CrossOrigin
public class DataAllPatentController {

    @Autowired
    private DataAllDrugPatentService dataAllDrugPatentService;

    @Autowired
    private DataAllDrugPatentRepository dataAllDrugReposity;

    @GetMapping("/findById")
    public Map<String , Object> findById(@RequestParam Integer id){

        Map<String, Object> map = new HashMap<>();
        Optional<data_all_drug_patent> r = dataAllDrugReposity.findById(id);
        if(r.isPresent()){
            map.put("code", "200");
            map.put("result", r);
        }else{
    ...
    public Map<String, Object> update(
        ...
        data_all_drug_patent dad = new data_all_drug_patent(id,
...
//        if(symptom_zy_zgyyxxcxpt==0)s_z_batch=0;
        ...
        PartColumsPatent p=new PartColumsPatent(status,name,yaobw,zyybd,baike);
    ...
    private Map<String,Object> deleteById(@RequestParam int id){
        ...
        dataAllDrugReposity.deleteById(id);
        map.put("code","200");
        return map;
    }
