...
import com.example.demo1.repository.index.DiseaseZyIndexRepository;
...
@RestController
@RequestMapping("/diseaseZyIndex")
@CrossOrigin(origins = "http://localhost:8080")
public class DiseaseZyIndexController {
    @Autowired
    private DiseaseZyIndexRepository diseaseZyIndexRepository;
    ...
    private Map<String,Object> searchPatent(@RequestParam(required = false,defaultValue = "*") String content){
        ...
        NativeSearchQuery searchQuery=new NativeSearchQueryBuilder()
                .withQuery(
                        QueryBuilders.boolQuery().must(QueryBuilders.multiMatchQuery(content,"info_lcbx","info_jbzd","info_xybm"))
                ).build();
        Iterable<DiseaseZyIndex> result=diseaseZyIndexRepository.search(searchQuery);
        ...
        set.forEach(ea->{
            data_all_symptom_disease_zxy da=dataAllSymptomDiseaseZxyRepository.findInDiseaseZy(ea);
