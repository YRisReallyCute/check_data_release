...
public interface DataAllDrugPatentRepository extends JpaRepository<data_all_drug_patent, Integer>, JpaSpecificationExecutor<data_all_drug_patent>,CrudRepository<data_all_drug_patent, Integer> {

    @Transactional
    @Query(value = "select * from data_all_drug_patent where info_ym like CONCAT('%',?1,'%')",nativeQuery = true)
    List<data_all_drug_patent> findByName(String name);

    @Transactional
    @Modifying
    @Query(value = "update data_all_drug_patent set status=4 where id=?1",nativeQuery = true)
    void deleteById(int id);


    @Query(value = "select * from data_all_drug_patent where info_ym like ?1",nativeQuery = true)
    data_all_drug_patent findByYm(String name);

    @Modifying
    @Transactional
    @Query(value = "update data_all_drug_patent a set " +
            ...
            "where a.id = :#{#u.id}")
    int updateCommentAndStatus(@Param("u") data_all_drug_patent u);
